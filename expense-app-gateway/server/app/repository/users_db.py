...
async def create_users(user: user_model.Users ):
    ...
    try:
        ...
        statement = await con.fetch(
            "INSERT INTO users  (public_id,email,prefix_mobile,mobile_number,activated,note,created_at,updated_at,deleted_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id",
            str(uuid.uuid4()),
            user.email,
            user.prefix_mobile,
            user.mobile_number,
...
async def update_users(user_id, user: user_model.Users ):
    ...
    try:
        await tr.start()
        statement = await con.fetch(
            "UPDATE users SET email = $1,prefix_mobile = $2,mobile_number = $3,activated = $4,updated_at = $5 WHERE id = $6 RETURNING id",
            user.email,
            user.prefix_mobile,
            user.mobile_number,
            user.activated,
