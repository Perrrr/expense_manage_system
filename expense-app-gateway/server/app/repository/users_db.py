...
async def create_users(user: user_model.Users ):
    ...
    try:
        ...
        statement = await con.fetch(
            "INSERT INTO users (public_id,email,prefix_mobile,mobile_number,activated,created_at,updated_at,deleted_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING user_id",
            str(uuid.uuid4()),
            user.email,
            user.prefix_mobile,
            user.mobile_number,
    ...
    if statement:
        ...
        return ({
            "message": {
                "user_id": ((statement[0].get("user_id"))),
            }
        })
...
async def get_all_users_paginated(page_number, page_size):
    con = await create_connection()
    statement = await con.fetch(
        "SELECT * FROM users WHERE deleted_at IS NULL ORDER BY user_id DESC LIMIT $1 OFFSET $2",
        page_size,
...
async def update_users(user_id, user : user_model.Users ):
    ...
    try:
        await tr.start()
        statement = await con.fetch(
            "UPDATE users SET public_id = $1,email = $2,prefix_mobile = $3,mobile_number = $4,activated = $5,updated_at = $6 WHERE id = $7",
            user.public_id,
            user.email,
            user.prefix_mobile,
            user.mobile_number,
